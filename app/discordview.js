...
(function (root, factory) {
  ...
  const actionValue = value => value === 'true' ? true : value === 'false' ? false : value;
  const ICONS = {
  ...
  // A default volume of 100 when unset (both halves of a slider share this).
  const vol = v => v == null ? 100 : v;
  ...
  function richPresenceDisplay(activityState, enabled, connected) {
    if (activityState === 'available') return enabled
      ? { label: 'Active', detail: 'Publishing supported Bedrock Panel activity.' }
  ...
  function voiceView(state) {
    ...
    return '<section class="voice">'
      ...
      + '<article class="card participants"><div class="section-heading"><div><div class="eyebrow">Voice room</div><h2>Participants</h2></div><span class="count-pill">' + participants.length + '</span></div>' + (participants.length ? '<div class="participant-list" data-preserve-scroll="voice-participants">' + participants.map(p => {
        ...
        return '<div class="participant ' + (p.speaking ? 'speaking' : '') + '" data-scroll-item="participant-' + esc(p.id) + '">' + avatar(p, 'participant-avatar') + '<div class="participant-name"><strong>' + esc(displayName(p)) + '</strong><span>' + esc(participantState) + '</span></div>' + (participantControls ? '<button class="participant-mute ' + (p.localMute ? 'active-state' : '') + '" data-participant-action="participant-mute" data-user-id="' + esc(p.id) + '" data-value="' + (!p.localMute) + '" aria-pressed="' + (!!p.localMute) + '">' + (p.localMute ? 'Unmute' : 'Mute') + '</button><label class="participant-volume"><span>Vol</span><input aria-label="Participant volume" data-participant-action="participant-volume" data-user-id="' + esc(p.id) + '" type="range" min="0" max="200" value="' + esc(vol(p.volume)) + '"><output>' + esc(vol(p.volume)) + '%</output></label>' : '<span class="capability-note">Controls unavailable</span>') + '</div>';
      }).join('') + '</div>' : '<div class="empty-state">' + icon('user') + '<strong>No participants are exposed</strong><span>Discord has not supplied anyone for this voice channel.</span></div>') + (state.voiceControlLock ? '<p class="control-lock">Participant settings stay under Bedrock Panel control until RPC disconnects.</p>' : '') + '</article>'
