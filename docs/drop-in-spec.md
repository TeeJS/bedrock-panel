...
| `label` | Editor label. |
| `type` | `text` \| `bool` \| `secret` \| `select` \| `number` \| `folder` \| `file` \| (host-defined extras). `bool` is coerced to a real boolean; `number` is validated against `min`/`max`/`step` (below) but still delivered to the page as a **string** (the page coerces it). `folder` and `file` are text fields the editor pairs with a **Browse…** button opening the host's native directory / file picker; the stored value is a plain absolute path string, so a host that doesn't know the type still renders a usable text box. Any option holding a filesystem path SHOULD use one of these rather than `text` — a bare text field makes the user type a path by hand. |
...
| `min` / `max` / `step` | `type: "number"` only. Declared bounds and grid. The editor rejects a value below `min`, above `max`, or off the `step` grid. The grid is measured **from the base** (`min`, else `0`): `step: 1` means whole-unit increments from that base — integers only when the base is integral (e.g. `min: 0.5, step: 1` accepts `1.5`). Invalid drafts are kept (not silently defaulted) and **block Save across all pages** until corrected — the editor reveals and focuses the offending field. The page SHOULD apply matching strict validation at runtime (e.g. reject a supplied-but-invalid value rather than run at a wrong value). |
