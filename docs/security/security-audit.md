...
**Remediation outcome (updated 2026-08-26):** Resolved. The global token routes and renderer token getters remain removed. Microsoft 365 is now an installable drop-in whose manifest registers only `app:office`; its server module receives an OAuth facade already bound to that app id, and tokens remain in the main process. The former global Microsoft provider, Auth-tab entry, Office capability routes, and bundled Office runtime were removed. Legacy Microsoft tokens migrate once to `app:office` so existing users can reconnect or refresh without exposing credentials.
