...
- **`bedrock-panel_amd64.deb`** — Linux x64 for Debian and Ubuntu: installs a desktop entry and pulls
  its own dependencies. The knob and touchscreen need one udev rule; first-launch steps and what is
  not available on Linux: [docs/linux.md](docs/linux.md).
- **`bedrock-panel-x86_64.AppImage`** — Linux x64, portable, no install. Needs FUSE 2, which Ubuntu
  24.04 and newer no longer ship — install `libfuse2t64` or use the `.deb`.
