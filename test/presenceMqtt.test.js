...
test('safeNodeId makes a hostname safe for both MQTT topics and HA object ids', () => {
  ...
  assert.strictEqual(safeNodeId(''), 'bedrock-panel');
  assert.strictEqual(safeNodeId(null), 'bedrock-panel');
});

test('topics are built under the configured base and share one node id', () => {
  const t = buildTopics('bedrock-panel', 'TJ-PC');
  assert.strictEqual(t.node, 'tj-pc');
  assert.strictEqual(t.state, 'bedrock-panel/tj-pc/busy');
  assert.strictEqual(t.attributes, 'bedrock-panel/tj-pc/attributes');
  assert.strictEqual(t.availability, 'bedrock-panel/tj-pc/availability');
  assert.strictEqual(t.discovery, 'homeassistant/binary_sensor/tj-pc_bedrock_panel_busy/config');
});
...
test('stray slashes in the base topic do not produce empty topic levels', () => {
  ...
  assert.strictEqual(buildTopics('', 'pc').state, 'bedrock-panel/pc/busy');
});

test('discovery config carries what HA needs to auto-create a persistent entity', () => {
  const t = buildTopics('bedrock-panel', 'pc');
...
function svcWith(mqtt, over) {
  ...
  s.apply(Object.assign({ enabled: true, url: 'mqtt://broker:1883', baseTopic: 'bedrock-panel' }, over || {}));
  return s;
}
...
test('THE WILL is registered at connect — the whole crash failsafe depends on it', () => {
  ...
  assert.strictEqual(will.topic, 'bedrock-panel/tj-pc/availability');
...
test('on connect it announces discovery, availability, then state', () => {
  ...
  assert.ok(topics.includes('homeassistant/binary_sensor/tj-pc_bedrock_panel_busy/config'));
  assert.ok(topics.includes('bedrock-panel/tj-pc/availability'));
  assert.ok(topics.includes('bedrock-panel/tj-pc/busy'), 'current state replayed, not just discovery');
});
...
test('a reconnect re-announces, because a broker restart drops retained messages', () => {
  ...
  assert.strictEqual(mqtt._published.find(p => p.topic === 'bedrock-panel/tj-pc/busy').payload, 'ON');
});
...
test('state and attributes are published retained so HA survives a restart', () => {
  ...
  const state = mqtt._published.find(p => p.topic === 'bedrock-panel/tj-pc/busy');
  const attrs = mqtt._published.find(p => p.topic === 'bedrock-panel/tj-pc/attributes');
...
test('a clean stop says offline immediately rather than waiting for the will', () => {
  ...
  mqtt._published.length = 0;
  s.stop();
  const bye = mqtt._published.find(p => p.topic === 'bedrock-panel/tj-pc/availability');
...
test('re-applying identical settings does NOT reconnect', () => {
  ...
  mqtt._fire('connect');
  mqtt._published.length = 0;
  s.apply({ enabled: true, url: 'mqtt://broker:1883', baseTopic: 'bedrock-panel' });
...
test('changing the broker URL disconnects A and connects to B', () => {
  ...
  s.apply({ enabled: true, url: 'mqtt://right-host:1883', baseTopic: 'bedrock-panel' });
  ...
  // crash failsafe.
  assert.strictEqual(mqtt._opts().will.topic, 'bedrock-panel/tj-pc/availability');
});
...
test('changing only the credentials also reconnects', () => {
  ...
  mqtt._fire('connect');
  s.apply({ enabled: true, url: 'mqtt://broker:1883', baseTopic: 'bedrock-panel', username: 'new', password: 'pw' });
...
test('publishState before a connection is a no-op, and replays on connect', () => {
  ...
  s.apply({ enabled: true, url: 'mqtt://broker:1883', baseTopic: 'bedrock-panel' });
  mqtt._fire('connect');
  assert.ok(mqtt._published.some(p => p.topic === 'bedrock-panel/tj-pc/busy'),
    'the state held before connecting is announced once connected');
});

test('a base topic saved by a pre-rename install maps to the new default', () => {
  const t = buildTopics('open-quake', 'TJ-PC');
  assert.strictEqual(t.state, 'bedrock-panel/tj-pc/busy');
  assert.strictEqual(t.legacyDiscovery, 'homeassistant/binary_sensor/tj-pc_open_quake_busy/config');
});
